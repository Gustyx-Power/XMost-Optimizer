<script lang="ts">
	...
	async function fetchTelemetry() {
		try {
			...
			currentTimerResolutionMs = await invoke<number>(
	...
	async function loadAutoPurgeState() {
		try {
			const [enabled, standbyMb, freeMb] = await invoke<
				[boolean, number, number]
			>("get_auto_purge_state");
...
<div class="max-w-7xl mx-auto space-y-8 font-sans">
	...
	<header class="pb-2 border-b border-white/[0.02]">
		<h1 class="text-3xl font-bold tracking-tight text-text-primary">
			{locale.t("memory.title")}
		</h1>
		<p class="text-text-secondary text-xs mt-1">
			{locale.t("memory.subtitle")}
		</p>
	</header>
	...
	<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
		...
		<Card class="p-6 space-y-6 bg-bg-card rounded-3xl shadow-sm">
			<div class="flex items-center justify-between">
				<h2 class="text-xl font-normal text-text-primary">
					{locale.t("memory.auto_purge.title")}
				</h2>
				<div class="flex items-center gap-3">
					<span
						class="text-xs text-text-secondary uppercase tracking-wider font-semibold font-mono"
						>{autoPurgeEnabled
							? locale.t("common.enabled")
							: locale.t("common.disabled")}</span
					>
					<!-- MD3 Switch toggle -->
					<button
						onclick={toggleAutoPurge}
						class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full transition-colors duration-200 focus:outline-none {autoPurgeEnabled
							? 'bg-accent-cyan'
							: 'bg-text-muted/20'}"
						role="switch"
					...
					>
						<span
							class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition duration-200 ease-in-out {autoPurgeEnabled
								? 'translate-x-6 bg-bg-primary'
			...
			<div class="space-y-4">
				...
				<div class="space-y-2">
					<label
						for="standby-threshold"
						class="flex items-center justify-between text-sm text-text-secondary font-medium"
					>
						<span
							>{locale.t(
								"memory.auto_purge.threshold_standby",
							)}</span
						>
						<span class="font-mono text-accent-cyan font-bold"
							>{standbyListMb} MB</span
						>
					</label>
					<input
					...
					<p class="text-[11px] text-text-muted">
						{locale.t("memory.standby.title")}: {standbyListMb} MB
					</p>
				</div>
				...
				<div class="space-y-2">
					<label
						for="free-memory-threshold"
						class="flex items-center justify-between text-sm text-text-secondary font-medium"
					>
						<span
							>{locale.t(
								"memory.auto_purge.threshold_avail",
							)}</span
						>
						<span class="font-mono text-accent-cyan font-bold"
							>{freeMemoryMb} MB</span
						>
					</label>
					<input
					...
					/>
					<p class="text-[11px] text-text-muted">
						{locale.t("memory.free.title")}: {freeMemoryMb} MB
					</p>
				</div>
				...
				<div class="grid grid-cols-2 gap-4 pt-2">
					<div
						class="rounded-2xl p-4 bg-bg-primary-end border border-border-default"
					>
						<p class="text-xs text-text-secondary font-medium">
							{locale.t("common.available")}
						</p>
						<p
							class="text-lg font-normal text-text-primary mt-1 font-mono tracking-tight"
						>
		...
		<Card class="p-6 space-y-6 bg-bg-card rounded-3xl shadow-sm">
			<div class="flex items-center justify-between">
				...
				<div class="flex items-center gap-3">
					...
					<button
						...
						class="relative inline-flex h-6 w-11 shrink-0 cursor-pointer items-center rounded-full transition-colors duration-200 focus:outline-none {$timerEnabled
							? 'bg-accent-cyan'
							: 'bg-text-muted/20'}"
						role="switch"
					...
					>
						<span
							class="pointer-events-none inline-block h-4 w-4 transform rounded-full bg-white shadow-sm transition duration-200 ease-in-out {$timerEnabled
								? 'translate-x-6 bg-bg-primary'
								: 'translate-x-1 bg-text-secondary'}"
						></span>
					</button>
			...
			<div class="space-y-4">
				...
				<div
					class="rounded-2xl p-4 border border-accent-cyan/15 bg-bg-primary-end"
				>
					<p class="text-xs text-text-secondary font-medium mb-1">
						{locale.t("memory.timer.current")}
					</p>
					<p
						class="text-3xl font-normal text-accent-cyan font-mono tracking-tight"
					>
						{currentTimerResolutionMs.toFixed(2)} ms
					</p>
				</div>
				...
				<div class="space-y-2">
					...
					/>
					<p class="text-[11px] text-text-muted">
						{locale.t("memory.timer.desc")}
					</p>
				</div>

				<!-- Windows 11 Global Timer Requests (IAPS) -->
				<label
					class="flex items-center gap-4 cursor-pointer p-4 rounded-2xl bg-bg-primary-end border border-border-default hover:bg-bg-primary-end/80 transition-colors"
				>
					<input
						type="checkbox"
						bind:checked={$iapsEnabled}
						class="w-5 h-5 rounded accent-accent-cyan disabled:opacity-50"
						disabled={!$timerEnabled}
					/>
					<div class="flex-1">
						<p class="text-sm font-medium text-text-primary">
							Use GlobalTimerResolutionRequests
						</p>
		...
		<Card
			class="p-6 space-y-6 lg:col-span-2 bg-bg-card rounded-3xl shadow-sm"
		>
			<h2 class="text-xl font-normal text-text-primary">
				{locale.t("memory.pagefile.title")}
			</h2>
			...
			<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
				...
				<div class="space-y-3">
					<div class="flex items-center justify-between">
						<p class="text-sm text-text-secondary font-medium">
							{locale.t("memory.pagefile.usage")}
						</p>
						<p
							class="text-sm font-mono text-accent-purple font-bold"
						>
							{pagefilePercent.toFixed(1)}%
						</p>
					</div>

					<!-- Material 3 Thick Linear Progress Bar -->
					<div
						class="relative w-full h-3 rounded-full overflow-hidden bg-accent-purple/15"
					>
						<div
					...
					<div
						class="flex justify-between text-xs text-text-secondary font-mono mt-1"
					>
						<span
							>{pagefileUsedMb} MB {locale
								.t("memory.pagefile.used")
								.toLowerCase()}</span
						>
						<span
							>{pagefileTotalMb} MB {locale
								.t("common.total")
								.toLowerCase()}</span
						>
					</div>
				...
				<div class="space-y-3">
					...
					<p class="text-[11px] text-text-muted">
						How often to refresh telemetry data
					</p>
				</div>
			</div>
		</Card>
